...
require('lazy').setup({
		...
		opts = {
			...
			link = {
				...
				custom = {
					...
					gist = { pattern = '^http[s]?://gist%.github%.com/.*', icon = '󰅴 ', highlight = 'RenderMarkdownLink' },
					...
					hackmd = { pattern = "https://hackmd%.io/.*", icon = '󰏫 ', highlight = 'RenderMarkdownLink' },
					ithome = { pattern = '^http[s]?://.*%.ithome%.com%.tw/.*', icon = '幫|', highlight = 'RenderMarkdownLink' },
					...
					google_form = { pattern = '^http[s]?://docs%.google%.com/forms/.*', icon = '󱐁 ', highlight = 'RenderMarkdownLink' },
					google_slides = { pattern = '^http[s]?://docs%.google%.com/presentation/.*', icon = '󰈩 ', highlight = 'RenderMarkdownLink' },
					...
					claude = { pattern = '^http[s]?://.*claude%.ai/.*', icon = '󰚩 ', highlight = 'RenderMarkdownLink' },
					...
					google_drive = { pattern = '^http[s]?://drive%.google%.com/.*', icon = ' ', highlight = 'RenderMarkdownLink' },
					google_map = { pattern = '^http[s]?://www%.google%.com/maps/.*', icon = '󰍎 ', highlight = 'RenderMarkdownLink' },
					...
					notion2 = { pattern = '^http[s]?://.*%.notion%.site/.*', icon = '󰬕 ', highlight = 'RenderMarkdownLink' },
	...
	},
	{
		...
		config = function()
			-- add line count text after fold
